...
import { salesQuotationModel } from '../../../../models/trade_business/panel/sales/data_sales_quotations.js';
...
const SALES_ORDER_DOCTYPE_NAME = 'Sales Order';

const getSalesOrderDocTypeId = async () => {
  const rows = await tradeBusinessDbc.executeQuery(
    "SELECT id FROM master_doctype WHERE name = ? AND category = 'sales' LIMIT 1",
    [SALES_ORDER_DOCTYPE_NAME],
  );
  return rows?.[0]?.id || null;
};

const resolveDocumentChainIds = async (rootId) => {
  const ids = new Set();
  const guard = new Set();

  // Ancestors: walk base_entry upward.
  let cursor = rootId;
  while (cursor && !guard.has(cursor)) {
    guard.add(cursor);
    ids.add(cursor);
    const rows = await tradeBusinessDbc.executeQuery(
      'SELECT base_entry FROM sales_quotations WHERE id = ?',
      [cursor],
    );
    cursor = rows?.[0]?.base_entry || null;
  }

  // Descendants: BFS downward via base_entry.
  let frontier = [...ids];
  while (frontier.length > 0) {
    const placeholders = frontier.map(() => '?').join(', ');
    const children = await tradeBusinessDbc.executeQuery(
      `SELECT id FROM sales_quotations WHERE base_entry IN (${placeholders})`,
      frontier,
    );
    const next = [];
...
const resolveSalesOrderId = async (rootId) => {
  const soDocTypeId = await getSalesOrderDocTypeId();
  if (!soDocTypeId) return rootId;

  const chainIds = await resolveDocumentChainIds(rootId);
  if (chainIds.length === 0) return rootId;

  const placeholders = chainIds.map(() => '?').join(', ');
  const chainRows = await tradeBusinessDbc.executeQuery(
    `SELECT id, doc_type, base_entry FROM sales_quotations WHERE id IN (${placeholders})`,
    chainIds,
  );

  const docTypeById = new Map(chainRows.map((row) => [row.id, row.doc_type]));
  const baseEntryById = new Map(
    chainRows.map((row) => [row.id, row.base_entry]),
  );

  // Prefer self, then the nearest ancestor, then any descendant.
  let cursor = rootId;
  while (cursor) {
    if (docTypeById.get(cursor) === soDocTypeId) {
      return cursor;
    }
    cursor = baseEntryById.get(cursor) || null;
  }

  const found = chainRows.find((row) => row.doc_type === soDocTypeId);
  return found?.id || rootId;
};
...
export const getSalesQuotationPurchaseCosts = catchAsync(
  async (req, res, next) => {
    ...
    // 1. Resolve the Sales Order this document belongs to, then fetch its
    //    purchase requests (PRs are linked to the Sales Order).
    const salesOrderId = await resolveSalesOrderId(sales_quotation_id);
    ...
    const purchaseRequests = await tradeBusinessDbc.executeQuery(
      ...
      [salesOrderId, 'ap_invoiced'],
    );

    if (purchaseRequests.length === 0) {
      return res.status(200).json({
        status: 'success',
        sales_quotation_id: salesOrderId,
        requested_sales_quotation_id: sales_quotation_id,
        shipping_costs: [],
        product_costs: [],
        service_costs: [],
      });
