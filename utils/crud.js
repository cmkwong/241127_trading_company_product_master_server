...
class CrudOperations {
  ...
  static async performCrud(options) {
    try {
      ...
      if (schema.length === 0) {
        throw new AppError(`Table ${tableName} not found or has no columns`, 400);
      }
      
      // Get primary key column(s)
      const primaryKeys = schema.filter(col => col.COLUMN_KEY === 'PRI').map(col => col.COLUMN_NAME);
      ...
      const hasCreatedAt = schema.some(col => col.COLUMN_NAME === 'created_at');
      const hasUpdatedAt = schema.some(col => col.COLUMN_NAME === 'updated_at');
      const hasDeletedAt = schema.some(col => col.COLUMN_NAME === 'deleted_at');
      ...
      switch (operation.toLowerCase()) {
        case 'create':
          result = await this.createRecord(connection, tableName, schema, data, hasCreatedAt, hasUpdatedAt);
          break;
          
        case 'bulkcreate':
          result = await this.bulkCreateRecords(connection, tableName, schema, data, hasCreatedAt, hasUpdatedAt);
          break;
        ...
        case 'update':
          result = await this.updateRecord(connection, tableName, schema, id, data, hasUpdatedAt);
          break;
          
        case 'bulkupdate':
          result = await this.bulkUpdateRecords(connection, tableName, schema, data, primaryKeys[0], hasUpdatedAt);
          break;
          
        case 'delete':
          result = await this.deleteRecord(connection, tableName, schema, id, softDelete, hasDeletedAt);
          break;
          
        case 'bulkdelete':
          result = await this.bulkDeleteRecords(connection, tableName, schema, id, softDelete, hasDeletedAt);
          break;
      ...
      throw new AppError(`CRUD operation failed: ${error.message}`, error.statusCode || 500);
    }
  }
   ...
   */
  static async createRecord(connection, tableName, schema, data, hasCreatedAt, hasUpdatedAt) {
    if (!data) {
    ...
    const idColumn = schema.find(col => col.COLUMN_KEY === 'PRI' && col.COLUMN_NAME.toLowerCase().includes('id'));
    if (idColumn && !recordData[idColumn.COLUMN_NAME] && idColumn.DATA_TYPE.includes('char')) {
    ...
    const createdRecord = await this.readRecords(connection, tableName, schema, insertId);
   ...
   */
  static async bulkCreateRecords(connection, tableName, schema, dataArray, hasCreatedAt, hasUpdatedAt) {
    ...
    dataArray.forEach(data => {
      ...
      // Generate UUID for ID field if needed
      if (idColumn && !recordData[idColumnName] && idColumn.DATA_TYPE.includes('char')) {
      ...
      columnArray.forEach(column => {
        recordValues.push(recordData[column] !== undefined ? recordData[column] : null);
      });
    ...
    const placeholderGroups = allValues.map(() => `(${columnArray.map(() => '?').join(', ')})`);
    ...
    if (createdIds.length > 0) {
      ...
      createdRecords = await this.readRecords(connection, tableName, schema, createdIds);
    } else if (result.insertId) {
      // Otherwise, fetch a range based on insertId
      const ids = Array.from({ length: dataArray.length }, (_, i) => result.insertId + i);
      createdRecords = await this.readRecords(connection, tableName, schema, ids);
    }
  ...
  static async readRecords(
    ...
    // Add ID condition
    if (id) {
      const primaryKeyColumn = schema.find(col => col.COLUMN_KEY === 'PRI').COLUMN_NAME;
      
      if (Array.isArray(id)) {
        // For multiple IDs
        const idPlaceholders = id.map(() => '?').join(', ');
        const idCondition = `${primaryKeyColumn} IN (${idPlaceholders})`;
        whereClause = whereClause ? `${whereClause} AND ${idCondition}` : idCondition;
        whereParams.push(...id);
      } else {
        // For single ID
    ...
    if (conditions) {
      ...
      if (conditionClauses.length > 0) {
        ...
        whereClause = whereClause ? `${whereClause} AND (${conditionsStr})` : conditionsStr;
      }
    }
    ...
    if (!id || Array.isArray(id)) {
      ...
      const countResult = await dbModel.executeQuery(connection, countSQL, whereParams);
   ...
   */
  static async updateRecord(connection, tableName, schema, id, data, hasUpdatedAt) {
    if (!id) {
    ...
    schema.forEach(column => {
      ...
      if (columnName !== primaryKeyColumn && updateData[columnName] !== undefined) {
    ...
    const updatedRecord = await this.readRecords(connection, tableName, schema, id);
   ...
   */
  static async bulkUpdateRecords(connection, tableName, schema, dataArray, idField, hasUpdatedAt) {
    if (!Array.isArray(dataArray) || dataArray.length === 0) {
    ...
    if (missingIds.length > 0) {
      throw new AppError(`${missingIds.length} records are missing the ID field (${idField})`, 400);
    }
    ...
    try {
      ...
      const updatedRecords = await this.readRecords(connection, tableName, schema, updatedIds);
   ...
   */
  static async deleteRecord(connection, tableName, schema, id, softDelete, hasDeletedAt) {
    if (!id) {
    ...
    const recordToDelete = await this.readRecords(connection, tableName, schema, id);
    
    if (!recordToDelete.record) {
      throw new AppError(`Record with ID ${id} not found in ${tableName}`, 404);
    }
    ...
    if (softDelete && hasDeletedAt) {
      ...
      await dbModel.executeQuery(connection, sql, [new Date(), id]);
      ...
      await dbModel.executeQuery(connection, sql, [id]);
   ...
   * @returns {Promise<Object>} Promise that resolves with deletion result
   */
  static async bulkDeleteRecords(connection, tableName, schema, ids, softDelete, hasDeletedAt) {
    ...
    const recordsToDelete = await this.readRecords(connection, tableName, schema, ids);
    
    if (recordsToDelete.records.length === 0) {
      throw new AppError(`No records found with the provided IDs in ${tableName}`, 404);
    }
    ...
    if (softDelete && hasDeletedAt) {
      ...
      await dbModel.executeQuery(connection, sql, [new Date(), ...ids]);
      
      return {
        message: `${ids.length} records soft deleted successfully from ${tableName}`,
        count: ids.length,
        ids,
      ...
      await dbModel.executeQuery(connection, sql, ids);
      
      return {
        message: `${ids.length} records deleted successfully from ${tableName}`,
        count: ids.length,
        ids,
        deletedRecords: recordsToDelete.records
      };
