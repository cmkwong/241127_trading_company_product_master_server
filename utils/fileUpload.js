...
export const saveBase64File = async (base64Data, uploadDir, options = {}) => {
  try {
    ...
    if (!isAllowedFileType(mimeType, allowedTypes)) {
      throw new AppError(`Unsupported file type. Allowed types: ${allowedTypes.join(', ')}`, 400);
    }
    ...
    if (fileSizeInMB > maxSizeInMB) {
      throw new AppError(`File size exceeds the maximum allowed size of ${maxSizeInMB}MB`, 400);
    }
    ...
    throw new AppError(`Failed to save file: ${error.message}`, error.statusCode || 500);
  }
};
...
export const deleteFile = async (filePath) => {
  try {
    ...
    console.error(`Warning: Failed to delete file at ${filePath}: ${error.message}`);
    return false;
  }
};
